...
async function main() {
  ...
  // Load previous snapshot to calculate closed conversations
  const snapshotsDir = './snapshots';
  let previousSnapshot = null;
  if (fs.existsSync(snapshotsDir)) {
    const files = fs.readdirSync(snapshotsDir)
      .filter(f => f.startsWith('snapshot-2026-04-22-') && f !== `snapshot-2026-04-22-${snapshot.timestamp.split('.')[0].replace(/[-:]/g, '')}.json`)
      .sort()
      .reverse();
    if (files.length > 0) {
      try {
        previousSnapshot = JSON.parse(fs.readFileSync(`${snapshotsDir}/${files[0]}`, 'utf8'));
      } catch (e) {
        console.warn('  ⚠  Could not load previous snapshot');
      }
    }
  }

  // Calculate closed conversations (E and F)
  let totalClosedE = 0, totalClosedF = 0;
  const closedByInbox = {};

  if (previousSnapshot && previousSnapshot.conversations) {
    const currentIds = new Set(allConversations.map(c => c.id));
    const closedConvos = previousSnapshot.conversations.filter(c => !currentIds.has(c.id));

    totalClosedE = closedConvos.filter(c => !c.admin_assignee_id).length;
    totalClosedF = closedConvos.filter(c => c.admin_assignee_id).length;

    // Calculate per inbox
    for (const result of results) {
      if (!result.teamId) continue;
      const inboxClosed = closedConvos.filter(c => c.team_assignee_id == result.teamId);
      closedByInbox[result.name] = {
        closedE: inboxClosed.filter(c => !c.admin_assignee_id).length,
        closedF: inboxClosed.filter(c => c.admin_assignee_id).length,
      };
    }
  }
  ...
  const output = {
    ...
    inboxes: results.map(inbox => ({
      ...inbox,
      closedE: closedByInbox[inbox.name]?.closedE || 0,
      closedF: closedByInbox[inbox.name]?.closedF || 0,
    })),
    unmapped: [],
  };
